...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleShortCodeNotFound(ShortCodeNotFoundException ex) {
        ...
        problem.setType(URI.create("urn:problem:short-code-not-found"));
    ...
    public ProblemDetail handleAliasConflict(AliasAlreadyExistsException ex) {
        ...
        problem.setType(URI.create("urn:problem:alias-conflict"));
    ...
    public ProblemDetail handleValidation(MethodArgumentNotValidException ex) {
        Map<String, String> fieldErrors = ex.getBindingResult().getFieldErrors().stream()
            .collect(Collectors.toMap(
                ...
                (a, b) -> a   // keep first message if same field has multiple violations
            ));
